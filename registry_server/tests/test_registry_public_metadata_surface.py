...
def test_anonymous_public_routes_do_not_emit_profile_metadata(client, path):
    ...
    forbidden_fields = ("provider", "license", "trust")
    if path != "/openapi.json":
        # The OpenAPI schema may name the deliberately public list fields. The
        # sensitive contract is that private provider/license/trust material and
        # concrete profile values never appear on anonymous routes.
        forbidden_fields = ("checksum", "download_url", *forbidden_fields)
