...
def test_inv4_unmodelled_skip_paths_are_surfaced() -> None:
    ...
    CONDITIONAL, and as of 2026-08-02 NO LONGER VACUOUS on the real tree. It was
    vacuous while the repo carried ZERO real strict-xfail marks
    (tests/test_smoke_e2e.py deliberately uses strict=False — see 15cd3a1). It is
    now LIVE for exactly ONE file: `tests/test_profile_key_redemption.py`, which
    carries 4 real `strict=True` xfails pinning open F20 findings. The per-file
    surface really does run for it — measured, not assumed: the detector reports
    `[]` mechanisms for that file, and it reports them because that file reached
    `unmodelled_skip_mechanisms` at all, which only files with a real strict mark
    do.

    That file originally used a runtime `pytest.skip(...)` for its absent
    cross-repo checkout; this invariant caught it, and it was converted to a
    declarative `@pytest.mark.skipif` — visible to INV-4's mark walk — rather than
    this count being relaxed. That is the intended direction of travel: resolve
    the surface, then move the count.

    Non-vacuity for every OTHER named mechanism is still proven by
    `test_inv4_unmodelled_surface_positive_control`, which drives the same
    detector with synthetic input.

    If this count changes again, do what this docstring's predecessor asked: read
    the newly-listed files, confirm the per-file surface ran for them, and only
    ...
    assert counts["files_with_strict_xfail"] == 1, (
    ...
    # Pin WHICH file, not just how many. A bare count is satisfied by any file
    # swapping in for any other, so it would let the live surface silently move
    # to a module nobody re-read — which is the exact review step the count is
    # here to force.
    assert counts["strict_xfail_files"] == ["tests/test_profile_key_redemption.py"], (
        f"the set of files carrying a real strict-xfail mark has changed to "
        f"{counts['strict_xfail_files']}. Re-read the docstring above, confirm the "
        f"per-file surface ran for each newly-listed file, then update both this "
        f"list and the count."
    )
